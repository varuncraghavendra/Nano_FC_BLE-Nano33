...
float alt_setpoint = 10.;
float pid_error_alt = 0.;
float pid_prev_error_alt = 0.;
float pid_integrator_alt = 0.;
float pid_output_alt = 0.;
...
void loop() {
  ...
  //Alt input
  if(rc_channels[2] > 1100 && rc_channels[2] < 1800) alt_setpoint = 0.27143 * rc_channels[2] - 288.57;
  ...
  //4) Alt 
  pid_error_alt = height_cm - alt_setpoint;
  pid_integrator_alt += KI_ALT * pid_error_alt;
  if(pid_integrator_alt > PID_MAX_OUTPUT) pid_integrator_alt = PID_MAX_OUTPUT;
  else if(pid_integrator_alt < -PID_MAX_OUTPUT) pid_integrator_alt = -PID_MAX_OUTPUT;

  pid_output_alt = KP_ALT * pid_error_alt + pid_integrator_alt + KD_ALT * (pid_error_alt - pid_prev_error_alt);
  if(pid_output_alt > PID_MAX_OUTPUT) pid_output_alt = PID_MAX_OUTPUT;
  else if(pid_output_alt < -PID_MAX_OUTPUT) pid_output_alt = -PID_MAX_OUTPUT;

  pid_prev_error_alt = pid_error_alt;
  
  if (uav_start_flag == 2) {

    if(rc_channels[2] > 1800) rc_channels[2] = 1800;
    des_throttle = rc_channels[2] - pid_output_alt;
    if (des_throttle > 1800) des_throttle = 1800;

    esc_pwm[0] = des_throttle + pid_output[0] - pid_output[1] - pid_output[2];
    esc_pwm[1] = des_throttle - pid_output[0] + pid_output[1] - pid_output[2];
    esc_pwm[2] = des_throttle - pid_output[0] - pid_output[1] + pid_output[2];
    esc_pwm[3] = des_throttle + pid_output[0] + pid_output[1] + pid_output[2];
...
void read_tfmini(){
  if(Serial1.available() && Serial1.read() == 0x59 && Serial1.read() == 0x59){
    ...
    tfmini_lsb = Serial1.read();
    tfmini_usb = Serial1.read();
    raw_height_cm = (int)tfmini_usb * 256 + (int)tfmini_lsb;
